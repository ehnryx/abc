...
struct ford_fulkerson {
  ...
  auto add_edge(int a, int b, const T& c = 1, bool bidirectional = false) -> void {
  ...
  auto flow(int source, int sink, int max_iters = std::numeric_limits<int>::max()) -> T {
    if constexpr (!std::is_integral_v<T>) assert(max_iters < std::numeric_limits<int>::max());
